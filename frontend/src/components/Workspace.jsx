...
import { 
  Terminal, Users, Code, ArrowLeft, Copy, Check, 
  History, Calendar, Trash2, GitMerge, MessageSquare, Send, CheckCircle, HelpCircle
} from 'lucide-react';
...
export const Workspace = ({ roomId, onLeaveRoom }) => {
  ...
  const [codeMerged, setCodeMerged] = useState('// Panel 3: Collaborative resolution panel');
  ...
  useEffect(() => {
    ...
    socket.on('room-state', (state) => {
      ...
      try {
        ...
        if (editorRef1.current && editorRef1.current.getValue() !== state.code1) {
          editorRef1.current.setValue(state.code1);
        }
        if (editorRef2.current && editorRef2.current.getValue() !== state.code2) {
          editorRef2.current.setValue(state.code2);
        }
        if (editorRef3.current && editorRef3.current.getValue() !== state.codeMerged) {
    ...
    socket.on('cursor-sync', ({ username, color, panel, position }) => {
      ...
      decorationsRef.current = targetEditor.deltaDecorations(decorationsRef.current, newDecorations);
    });
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < diffs.length; i++) {
      ...
      if (part.removed) {
        ...
        const className = isConflict ? 'diff-line-conflict' : 'diff-line-removed';
        const gutterName = isConflict ? 'diff-gutter-conflict' : 'diff-gutter-removed';
        ...
        const gutterName = isConflict ? 'diff-gutter-conflict' : 'diff-gutter-added';
    ...
    diffDecorationsRef1.current = editor1.deltaDecorations(diffDecorationsRef1.current, monacoRange1);
    diffDecorationsRef2.current = editor2.deltaDecorations(diffDecorationsRef2.current, monacoRange2);
  ...
  const getPanelEditorLabel = (panel) => {
    if (panel === 1) {
      ...
      return user1 === user.username ? 'You (Left Developer)' : `${user1}'s Panel (Read-only)`;
    }
    if (panel === 2) {
      if (!user2) return 'Awaiting Developer 2...';
      return user2 === user.username ? 'You (Right Developer)' : `${user2}'s Panel (Read-only)`;
    }
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          ...
          >
            ...
            <strong style={{ letterSpacing: '0.05em', color: 'var(--text-primary)' }}>{roomId}</strong>
            {copied ? <Check size={14} style={{ color: '#10b981' }} /> : <Copy size={14} style={{ color: 'var(--text-secondary)' }} />}
          </div>
        ...
        >
          ...
          <div style={{ display: 'flex', flex: 1, gap: '16px', minHeight: 0 }}>
            ...
            >
              ...
              >
                ...
                {!isEditable(1) && (
                  <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', background: 'rgba(255,255,255,0.04)', padding: '2px 8px', borderRadius: '4px' }}>
                    Read-Only
                  </span>
                )}
              </div>
              <div style={{ flex: 1, position: 'relative' }}>
                <Editor
                  ...
                  onMount={(editor) => {
                    ...
                    editor.onDidChangeCursorPosition(() => handleCursorMove(1, editor));
              ...
              <div style={{ flex: 1, position: 'relative' }}>
                <Editor
                  ...
                  onMount={(editor) => {
                    ...
                    editor.onDidChangeCursorPosition(() => handleCursorMove(2, editor));
                    // Force layout recalculation after mounting to align highlights perfectly
                    setTimeout(() => {
                      editor.layout();
                    }, 500);
          ...
          >
            ...
            >
              ...
              <span style={{ fontSize: '0.75rem', color: '#10b981', background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.2)', padding: '2px 8px', borderRadius: '4px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#10b981', display: 'inline-block' }} className="pulse-glow" />
                Live Synced
              </span>
            </div>
            <div style={{ flex: 1, position: 'relative' }}>
              <Editor
                ...
                onMount={(editor) => {
                  ...
                  editor.onDidChangeCursorPosition(() => handleCursorMove(3, editor));
                  // Force layout recalculation after mounting to align highlights perfectly
                  setTimeout(() => {
                    editor.layout();
                  }, 500);
          ...
          >
            <button
              ...
              style={{
                ...
                fontSize: '0.8rem',
                borderRadius: '6px',
                background: activeTab === 'resolver' ? 'rgba(99, 102, 241, 0.15)' : 'transparent',
                border: '1px solid ' + (activeTab === 'resolver' ? 'rgba(99, 102, 241, 0.25)' : 'transparent'),
                color: activeTab === 'resolver' ? 'var(--text-primary)' : 'var(--text-secondary)',
              }}
            >
              <GitMerge size={14} />
              Merge Helper
            ...
            <button
              ...
              style={{
                ...
                border: '1px solid ' + (activeTab === 'chat' ? 'rgba(99, 102, 241, 0.25)' : 'transparent'),
            ...
            >
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <h4 style={{ fontSize: '0.95rem', fontWeight: 600 }}>Conflict Resolver</h4>
                  <span
                    style={{
                      fontSize: '0.75rem',
                  ...
                  >
                    {conflictBlocks.filter(b => b.type !== 'common').length} differences
                  </span>
                </div>
                ...
                >
                  ...
                  />
                  Show Differences Only ({conflictBlocks.filter(b => b.type !== 'common').length})
                </label>
              </div>
              ...
              {conflictBlocks.length === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {conflictBlocks
                    .filter((block) => !showOnlyDiffs || block.type !== 'common')
                    .map((block, index) => {
                      ...
                      return (
                        ...
                        >
                          <strong style={{ fontSize: '0.8rem', color: isConflict ? '#f59e0b' : isRemoved ? '#f87171' : isCommon ? 'var(--text-muted)' : '#34d399' }}>
                            {title}
                          </strong>

                          <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
                            {isConflict ? (
                              <>Lines Left: {block.start1}-{block.end1} | Right: {block.start2}-{block.end2}</>
                            ) : isRemoved ? (
                          ...
                          <div style={{ display: 'flex', gap: '8px', marginTop: '4px' }}>
                            {isCommon && (
                              <button
                                ...
                                style={{ flex: 1, textAlign: 'center', background: 'rgba(255, 255, 255, 0.05)', borderColor: 'rgba(255, 255, 255, 0.1)', color: 'var(--text-primary)' }}
                              >
                                Accept Common Block
                              </button>
                            )}
                            ...
                            {isConflict && (
                              <button
                                onClick={() => handleAcceptBlock(block.text1 + block.text2)}
                                className="diff-resolve-badge"
                                style={{
                                  flex: 1.2,
                                  textAlign: 'center',
                ...
                {messages.length === 0 ? (
                  ...
                  messages.map((msg, index) => {
                    ...
                    return (
                      ...
                      >
                        {/* Sender Label */}
                        <span style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', marginBottom: '2px', padding: '0 4px' }}>
                        ...
                        <div
                          style={{
                            background: isSelf ? 'var(--accent-glowing)' : 'rgba(255, 255, 255, 0.05)',
                            border: '1px solid ' + (isSelf ? 'transparent' : 'var(--border-color)'),
                            padding: '8px 12px',
                            borderRadius: isSelf ? '12px 12px 2px 12px' : '12px 12px 12px 2px',
                            color: '#ffffff',
                            fontSize: '0.85rem',
                            wordBreak: 'break-word',
                            boxShadow: isSelf ? '0 2px 8px rgba(99, 102, 241, 0.25)' : 'none',
                          }}
                        >
                          {msg.text}
                        </div>
